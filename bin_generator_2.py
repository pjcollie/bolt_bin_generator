...
class BoltBinApp:
    ...
    def setup_gui(self):
        ...
        Tooltip(material_menu, "Select material for the entire bin (cannot mix materials)")
        ...
        self.size_entry.bind("<Return>", lambda event: self.length_entry.focus_set())  # Enter moves to length
        ...
        self.length_entry.bind("<Return>", lambda event: self.add_bolt())  # Enter triggers add_bolt
        ...
        Tooltip(remove_btn, "Remove the last added bolt size and length")
        ...
        self.canvas.grid(row=8, column=0, columnspan=2, padx=10, pady=10, sticky="ew")  # Center horizontally
    ...
    def reset_on_material_or_size_change(self, event):
        """Reset bolts when material or bin size changes."""
        if self.bolts:
            self.bolts = []
            self.output.delete(1.0, tk.END)
            self.canvas.delete("all")
            messagebox.showinfo("Reset", "Bolts cleared due to material or bin size change. All bolts must use the same material.")
    ...
    def add_bolt(self):
        ...
        self.update_output()
        self.update_grid()
        self.size_entry.focus_set()  # Return focus to size_entry after adding

    def remove_last_item(self):
        """Remove the last added bolt size and length."""
        if not self.bolts:
            messagebox.showinfo("Info", "No bolts to remove!")
            return
        # Remove the last length from the last size
        last_bolt = self.bolts[-1]
        last_bolt[1].pop()  # Remove the last length
        if not last_bolt[1]:  # If no lengths remain, remove the size
            self.bolts.pop()
        self.update_output()
        self.update_grid()
        messagebox.showinfo("Success", "Last bolt removed.")
    ...
    def update_grid(self):
        ...
        cell_width = 40
        cell_height = 25
        offset_x, offset_y = 80, 30  # Increased offset_x to prevent clipping
        ...
        for i, (size, _) in enumerate(self.bolts[:rows], 0):
            size_str = self.format_number(size)
            self.canvas.create_text(offset_x - 50, offset_y + i * cell_height + cell_height / 2,
                                   text=f"{size_str}\"", anchor="e")  # Increased offset for labels
